...
You can install the module using the manifest URL:

<your module manifest URL here>
...
Поле «Поиск…» наверху фильтрует:

плейлисты по имени

треки по имени

алиасы треков (в виртуальном избранном)

Совпадающие плейлисты раскрываются автоматически.
...
избранные поднимаются наверх

сортируются по алфавиту

получают золотую подсветку

❤️ Избранные треки (виртуальный плейлист)

Средняя кнопка мыши по треку:

добавляет его в виртуальный плейлист «Избранное»

независимо от того, в каком плейлисте он находится

при добавлении «Избранное» автоматически раскрывается
...
✏ Алиасы / переименование треков (только в «Избранном»)

В «Избранном»:

наведите на трек → появится ✎

нажмите ✎ → введите любое имя

Это имя:

отображается только в «Избранном»

не меняет оригинальный трек
...
появляется компактная панель громкости

изменяет реальный volume трека во Foundry

не закрывается, пока вы тянете мышью

закрывается только при щелчке вне

Работает и в обычных плейлистах, и в «Избранном».

🎨 Цвета плейлистов

ПКМ по плейлисту:

открыть всплывающее окно с цветами
...
Цветовая палитра настраивается через настройки модуля.
...
Используйте manifest URL:

<your module manifest URL here>

Вручную

Скачать release с GitHub

Разместить в папке:
Data/modules/ginzzzu-music-deck/

Активировать модуль в Manage Modules

🧩 Совместимость

Поддерживается Foundry VTT 13+

Работает со всеми стандартными аудиосистемами

Имеет улучшения при наличии Color Picker

📘 План развития

Автопрокрутка к «Избранному» при добавлении трека

Группировка по папкам (опционально)

Теги треков

Интеграция с Ambient Sounds (опционально)
